...
sampleQ <- function(n,K,p.w=1/2){
  ...
  if(I>=2){
    for(i in I:2){
      for(p in (j-K[i]+1):j){
        repeat{
          for(m in (j+1):(j+K[i-1])){
            Q[m,p] <- rbinom(1,1,p.w)
          }
          if(sum(Q[(j+1):(j+K[i-1]),p])>0){break}
        }
        if(i>2){
          for(m in (j+K[i-1]+1):n){
            Q[m,p] <- rbinom(1,1,p.w)
          }
        }
      }
...
sampleZ2 <- function(n){
  #n:: bigz
  #r:: bigz, uniform sampling in [1:n]
#   numbits <- as.integer(log2(n))+1
  numbits <- as.integer(log2(n-1))+1
  repeat{
    r.bit <- rbinom(numbits,1,1/2)
    r <- as.bigz(paste0("0b",paste0(r.bit,collapse="")))+1
    if(r<=n){break}
  }
  r
}
